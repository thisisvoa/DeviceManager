...
//		MsgQueue.getTcpLogqueue().add(tcpClientLog);
	 ...
	 * 获取客户端通道
	 */
	public  static Client getClientByMac(String mac) {
		Iterator<Map.Entry<String, Client>> it = clientMap.entrySet()
				.iterator();
		Client client = null;
		while (it.hasNext()) {
			Map.Entry<String, Client> entry = it.next();
			client = entry.getValue();
			String mac1 = client.getUser().getMac();
			if(null != mac1){
				String replaceAllZero = mac1.replaceAll("^(0+)", "");
				if (replaceAllZero.equalsIgnoreCase(mac)) {
